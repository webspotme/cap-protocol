...
- **G1 (Atomicity)** — A `commit` phase that fails partway MUST leave the event log in a consistent state. Specifically: if the commit event was successfully appended, the change is durably recorded even if the materialized resource file write or HEAD bump fails. Implementations MUST provide a recovery operation that replays the event log and reconciles the materialized resource cache. Recovery MAY be invoked on demand (e.g., `cap verify --recover` in the reference implementation) rather than automatically on every open — but consumers operating on a registry of unknown state MUST run recovery before any read.
